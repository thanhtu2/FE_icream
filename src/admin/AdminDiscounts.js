    ...
    useEffect(() => {
        ...
        const fetchDiscounts = async () => {
            try {
                ...
                console.error('Lỗi khi lấy danh sách mã giảm giá:', error.response?.data || error.message);
    ...
    const handleDelete = async (discountId) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa mã giảm giá này?')) {
            try {
                ...
                setDiscounts(discounts.filter(discount => discount.DiscountID !== discountId));
